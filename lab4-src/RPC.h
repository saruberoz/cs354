...
class RPCThread {
	...
	// to check incoming request for this thread
	int _incoming_request;
	
	// to check results are ready
	int _result_ready;

	// Server waits until a request takes place
// 	sema_t _sem_request;	
	cond_t _cond_request;
	...
	// another mutex for the threadServer, because if using the existing mutex it will mess up and create deadlock
	mutex_t _mutexReqRes;
...
// 	sema_t _sem_available_threads;
	cond_t _cond_available_threads;
